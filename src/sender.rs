...
impl<T> Sender<T> {
    ...
    pub fn try_send(&self, item: T) -> Result<(), TrySendError<T>> {
        match self.chan().try_send(item) {
            Ok(()) => Ok(()),
            Err((item, true)) => Err(TrySendError::Disconnected(item)),
    ...
    pub fn send(&self, item: T) -> Result<(), SendError<T>> {
        // Speculatively perform a try_send
        match self.try_send(item) {
            Ok(()) => Ok(()),
            Err(TrySendError::Disconnected(item)) => Err(SendError::Disconnected(item)),
            Err(TrySendError::Full(item)) => block_on(self.send_async(item)),
        }
    }

    #[cfg(feature = "time")]
    #[cfg_attr(docsrs, doc(cfg(feature = "time")))]
    pub async fn send_timeout_async(&self, item: T, timeout: Duration) -> Result<(), SendTimeoutError<T>> {
        ...
        let item = match self.try_send(item) {
            Ok(()) => return Ok(()),
            Err(TrySendError::Disconnected(item)) => return Err(SendTimeoutError::Disconnected(item)),
        ...
        select(
            ...
            Either::Left((_, send_fut)) => send_fut.try_reclaim().map(SendTimeoutError::Timeout).map(Err).unwrap_or(Ok(())),
            Either::Right((Ok(()), _)) => Ok(()),
            Either::Right((Err(SendError::Disconnected(item)), _)) => Err(SendTimeoutError::Disconnected(item)),
        }).await
    }

    #[cfg(feature = "time")]
    #[cfg_attr(docsrs, doc(cfg(feature = "time")))]
    pub async fn send_deadline_async(&self, item: T, deadline: Instant) -> Result<(), SendTimeoutError<T>> {
    ...
    pub fn send_timeout(&self, item: T, timeout: Duration) -> Result<(), SendTimeoutError<T>> {
        block_on(self.send_timeout_async(item, timeout))
    }

    #[cfg(feature = "time")]
    #[cfg_attr(docsrs, doc(cfg(feature = "time")))]
    pub fn send_deadline(&self, item: T, deadline: Instant) -> Result<(), SendTimeoutError<T>> {
        block_on(self.send_deadline_async(item, deadline))
    }
